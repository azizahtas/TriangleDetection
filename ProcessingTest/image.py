...
def main(argv):
    ...
    # read source
    image = readSourceImage(srcImageFile, outputState, useSharpen, useThreshold, useDistort, distortCoeffs,
                            useStaticThreshold, staticThreshold, equalizeHist)
    ...
    # detect triangles
    contourCount, rawTriCount, boundCount, arcCount, areaCount, finalCount, tris = findTriangles(image, minArcLength,
                                                                                                 maxArcLength, minArea,
                                                                                                 maxArea,
                                                                                                 polyApproxFactor,
                                                                                                 useBounds, bounds,
                                                                                                 outputState,
...
def readSourceImage(s, outputState, useSharpen, useThreshold, useDistort, distortCoeffs, useStaticThreshold,
                    staticThreshold, equalizeHist):
    # read input file
    img = cv2.imread(s)
    # write state image
    if outputState == True:
...
def findTriangles(img, minArc, maxArc, minArea, maxArea, PAF, UseBounds, BoundPoly, outputState, minLegLength,
                  maxLegLength, maxLegVar):
    rawContourCount = 0
    rawTriCount = 0
    boundTriCount = 0
    arclenTriCount = 0
    ...
    if outputState == True:
        ...
        if UseBounds == True:
            cv2.line(imgBounds, (BoundPoly[0][0][0], BoundPoly[0][0][1]), (BoundPoly[0][1][0], BoundPoly[0][1][1]),
                     stateBoundColor, stateLineWidth)
            cv2.line(imgBounds, (BoundPoly[0][1][0], BoundPoly[0][1][1]), (BoundPoly[0][2][0], BoundPoly[0][2][1]),
                     stateBoundColor, stateLineWidth)
            cv2.line(imgBounds, (BoundPoly[0][2][0], BoundPoly[0][2][1]), (BoundPoly[0][3][0], BoundPoly[0][3][1]),
                     stateBoundColor, stateLineWidth)
            cv2.line(imgBounds, (BoundPoly[0][3][0], BoundPoly[0][3][1]), (BoundPoly[0][0][0], BoundPoly[0][0][1]),
                     stateBoundColor, stateLineWidth)
    ...
    for i in range(0, len(contours)):
        ...
        if len(shape) == 3:
            if outputState == True:
                cv2.line(imgRawTris, (shape[0][0][0], shape[0][0][1]), (shape[1][0][0], shape[1][0][1]), stateColor,
                         stateLineWidth)
                cv2.line(imgRawTris, (shape[1][0][0], shape[1][0][1]), (shape[2][0][0], shape[2][0][1]), stateColor,
                         stateLineWidth)
                cv2.line(imgRawTris, (shape[2][0][0], shape[2][0][1]), (shape[0][0][0], shape[0][0][1]), stateColor,
                         stateLineWidth)
            ...
            if checkBounds(shape, UseBounds, BoundPoly) == True:
                ...
                if outputState == True:
                    cv2.line(imgBoundedTris, (shape[0][0][0], shape[0][0][1]), (shape[1][0][0], shape[1][0][1]),
                             stateColor, stateLineWidth)
                    cv2.line(imgBoundedTris, (shape[1][0][0], shape[1][0][1]), (shape[2][0][0], shape[2][0][1]),
                             stateColor, stateLineWidth)
                    cv2.line(imgBoundedTris, (shape[2][0][0], shape[2][0][1]), (shape[0][0][0], shape[0][0][1]),
                             stateColor, stateLineWidth)
                ...
                if arcLength > minArc and arcLength < maxArc:
                    ...
                    if outputState == True:
                        cv2.line(imgArcLen, (shape[0][0][0], shape[0][0][1]), (shape[1][0][0], shape[1][0][1]),
                                 stateColor, stateLineWidth)
                        cv2.line(imgArcLen, (shape[1][0][0], shape[1][0][1]), (shape[2][0][0], shape[2][0][1]),
                                 stateColor, stateLineWidth)
                        cv2.line(imgArcLen, (shape[2][0][0], shape[2][0][1]), (shape[0][0][0], shape[0][0][1]),
                                 stateColor, stateLineWidth)
                    ...
                    if area > minArea and area < maxArea:
                        ...
                        if outputState == True:
                            cv2.line(imgArea, (shape[0][0][0], shape[0][0][1]), (shape[1][0][0], shape[1][0][1]),
                                     stateColor, stateLineWidth)
                            cv2.line(imgArea, (shape[1][0][0], shape[1][0][1]), (shape[2][0][0], shape[2][0][1]),
                                     stateColor, stateLineWidth)
                            cv2.line(imgArea, (shape[2][0][0], shape[2][0][1]), (shape[0][0][0], shape[0][0][1]),
                                     stateColor, stateLineWidth)
                        ...
                        if (leglen1 > minLegLength and leglen2 > minLegLength and leglen3 > minLegLength) and (
                                leglen1 < maxLegLength and leglen2 < maxLegLength and leglen3 < maxLegLength):
                            legLengthTriCount += 1
                            # finalTriCount += 1

                            if outputState == True:
                                cv2.line(imgLegLength, (shape[0][0][0], shape[0][0][1]),
                                         (shape[1][0][0], shape[1][0][1]), stateColor, stateLineWidth)
                                cv2.line(imgLegLength, (shape[1][0][0], shape[1][0][1]),
                                         (shape[2][0][0], shape[2][0][1]), stateColor, stateLineWidth)
                                cv2.line(imgLegLength, (shape[2][0][0], shape[2][0][1]),
                                         (shape[0][0][0], shape[0][0][1]), stateColor, stateLineWidth)

                            if abs(leglen1 - leglen2) < maxLegVar and abs(leglen2 - leglen3) < maxLegVar and abs(
                                    leglen1 - leglen3) < maxLegVar:
                                legVarTriCount += 1
                                finalTriCount += 1
                                ...
                                if outputState == True:
                                    cv2.line(imgLegVar, (shape[0][0][0], shape[0][0][1]),
                                             (shape[1][0][0], shape[1][0][1]), stateColor, stateLineWidth)
                                    cv2.line(imgLegVar, (shape[1][0][0], shape[1][0][1]),
                                             (shape[2][0][0], shape[2][0][1]), stateColor, stateLineWidth)
                                    cv2.line(imgLegVar, (shape[2][0][0], shape[2][0][1]),
                                             (shape[0][0][0], shape[0][0][1]), stateColor, stateLineWidth)
